...
- The **shared core** (`01`, `02`, `03`, `07`, `08`) is roughly 60% of the total work and is
  required for *any single option*. It is not overhead -- it is the thing being assessed
  (front-end craft, repo structure, workflow modelling, CI/CD).
- Each option's own surface is then comparatively thin.

`09-build-order.md` sequences the work so that **the build is submittable at four checkpoints**.
...
`10-scope-and-risks.md` holds the cut list, in cut order.
